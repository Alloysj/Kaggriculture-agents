"""
Kaggriculture — Rule-Based Starter Agent (v6)
===============================================
...
RANCHER_COUNT_PER_ANIMAL = 2   # roughly 1 rancher slot per this many animals (rounded up)
RANCHER_COUNT_MAX = 3          # hard cap on crew slots reserved for ranching, regardless of animal count
ANIMAL_COUNT_CAP = 3           # hard cap on animals, regardless of land owned
...
def _rancher_action(pos, unit_inv, shed, farm, board_size, animal_target_count):
    """
    ...
    inferred purely from the current observation:
      ...
      2. Holding a bought animal, standing on the matching empty structure
         -> place it.
      3. Fewer structures than `animal_target_count` exist, and every
         existing structure already has an animal placed -> build another
         one. (Capped, not unconditional: a real ladder loss showed the
         old code could never manage more than one animal at all, since
         it only ever built when literally zero structures existed.)
    ...
    # 3. Build another structure if we're under the target count AND every
    # existing one already has an animal -- avoids building a second empty
    # structure while the first is still waiting to be filled.
...
def agent(obs, configuration=None):
    ...
    # livestock -- AND only if there's enough season left for a first
    # animal to ever pay off. Testing showed starting ranching on a
    # single 25-tile quadrant left too few crop workers to fight a weed
    # backlog, cratering the crop economy even though livestock itself
    # worked; the days-left check applies the same endgame-waste lesson
    # from the crop side to a brand new animal.
    ...
    ranch_active = already_expanded and (
        bool(structures) or (money >= ANIMAL_READY_CASH and enough_season_left_to_start)
    )

    # How many animals we're aiming for right now, scaled with land tier
    # (same "develop before expanding" pacing as crew size) and capped.
    # v7 could only ever manage one animal -- it only built a structure
    # when literally zero existed. A real ladder loss flagged this
    # directly ("model seems to only manage one cow and does not buy
    # more").
    animal_target_count = 0
    if ranch_active:
        animal_target_count = min(ANIMAL_COUNT_CAP, len(farm["unlocked_quadrants"]) - 1)
    rancher_count = 0
    if animal_target_count > 0:
        rancher_count = min(RANCHER_COUNT_MAX, -(-animal_target_count // RANCHER_COUNT_PER_ANIMAL))  # ceil div
    ...
    # units and different-enough target types that the crop-side
    # coordination below doesn't apply to them).
    crop_units = []  # units needing Phase 2 movement coordination
    ...
    if farmer_immediate is None:
        crop_units.append({"idx": "farmer", "pos": tuple(farm["farmer"]), "have_seed": farmer_have_seed})

    for i, hand_pos in enumerate(farm["hands"], start=1):
        if i in rancher_indices:
            unit_inv = inventories[i] if i < len(inventories) else {}
            action = _rancher_action(hand_pos, unit_inv, shed, farm, board_size, animal_target_count)
            if action == ["PASS"]:
        ...
        if immediate is not None:
            ...
            crop_units.append({"idx": i - 1, "pos": tuple(hand_pos), "have_seed": hand_have_seed})

    # Phase 2: for every crop-side unit with nothing to do right where it
    # stands, assign movement targets ONCE across all of them together, so
    # multiple units don't independently converge on the same tile. A real
    # lost ladder game showed roughly half of all unit-actions were pure
    # movement -- this is the fix for that, not a per-unit re-scan.
    if crop_units:
        harvestable, needs_water, weeded, plantable = _scan_targets(farm, board_size, plantable_crops, day)
        assignments = _assign_movement_targets(crop_units, harvestable, needs_water, weeded, plantable)
        for u in crop_units:
            target = assignments[u["idx"]]
            if target is None:
                action = ["PASS"]
            else:
                step = _step_toward(u["pos"][0], u["pos"][1], target[0], target[1])
